...
- `mcp.go` -- Streamable HTTP MCP server (47 tools) + helpers. Tool handlers are package-level `mcpFoo` funcs wired via `buildMCPServer()`; `withService` centralises bearer auth + `resolveWorkspace`. See the MCP section below
...
Fork-local addition (commit `d71173a`). Featmap exposes most of its workspace API as a **Model Context Protocol** server so local LLM agents can drive boards. End-user docs (full 47-tool table, color/status enums, bootstrap recipe) live in `readme.md` -- this section is the dev/architecture view.
