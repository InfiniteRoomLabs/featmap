...
# Individual steps
./build/build_webapp.sh             # npm install + npm run build in webapp/
...
cd webapp && npm start              # serves on :3000
...
- `*-api.go` -- HTTP handlers grouped by domain (`users-api`, `account-api`, `workspace-api`, `subscription-api`, `link-api`)
