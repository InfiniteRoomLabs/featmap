...
- `mcp.go` -- Streamable HTTP MCP server (56 tools) + helpers. Tool handlers are package-level `mcpFoo` funcs wired via `buildMCPServer()`; `withService` centralises bearer auth + `resolveWorkspace`. See the MCP section below
- `plane.go` / `plane-api.go` / `mcp_plane.go` / `plane_poller.go` / `plane_cli.go` -- Plane comment-sync (fork-local, backend-only v1). `plane.go` = `PlaneClient` REST wrapper (`X-API-Key`, paginated+page-bounded comment list, context-threaded) + AES-256-GCM credential crypto (conf `planeEncryptionKey`) + `CommentOrigin`/`SyncStatus` typed enums mirroring PG enum types. The sync engine is `service.SyncLink`/`SyncProject` (push local->Plane, pull Plane->local, dedupe by external id via `plane_comment_map`, echo-safe). `plane_poller.go` is the first background job: builds its OWN service+repo+tx per cycle (runs outside request middleware) with an outer `recover()` + per-connection SAVEPOINT+recover so one connection's `MustExec` panic can't crash the server. Migration `24_plane_sync.up.sql` adds the 3 tables + 2 enum types
...
Fork-local addition (commit `d71173a`). Featmap exposes most of its workspace API as a **Model Context Protocol** server so local LLM agents can drive boards. End-user docs (full 56-tool table, color/status enums, bootstrap recipe) live in `readme.md` -- this section is the dev/architecture view.
