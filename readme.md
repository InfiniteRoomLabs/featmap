...
- `query_board(workspace_id, project_id, filter)` -- run a jq (gojq) program over the full board JSON server-side and get back only the matched values wrapped as `{ "results": [...] }`. The board is built in memory, the filter runs over it, and only the transformed result is returned. The `filter` is required; an empty filter errors (use `get_board` for the whole board). A malformed filter returns a clean parse error. Because the caller controls the output shape via jq, `query_board`'s inner result is not schema-validated (the `{results}` envelope is). gojq is a full jq implementation; its regex uses Go RE2 (no lookaround/backreferences). Note: `query_board` reads the same data from the database as `get_board` -- the saving is in the response payload, not the DB load.
